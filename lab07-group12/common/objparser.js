"use strict";
class OBJLoader{
    static getMesh(name, path){
        if(MeshCache[name] !== undefined){
            return MeshCache[name];
        }
        MeshCache[name] = GLUtils.createMesh(name, gl.TRIANGLES, [], [], [], []);

        OBJLoader.loadTextFileAJAX(path,
                         function(fileContent){
                             let objData = OBJLoader.parse(fileContent)[0];
                             let tempMesh = GLUtils.createMesh("tempObjModel", gl.TRIANGLES,
                                                                objData.indices,
                                                                objData.vertices,
                                                                objData.normals,
                                                                objData.textureCoordinates);
                             let realMesh = MeshCache[name];
                             // TODO: I don't know why, but this causes an error when loading multiple models and trying to draw something
                             // that does not have a texcoord attribute.
